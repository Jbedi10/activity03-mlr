...
1. For each pair of variables, how would you describe the relationship graphically?
  Do any of the relationships look linear? pf score and pf expression seem to behave linear
  Are there any interesting/odd features (outliers, non-linear patterns, etc.)?
  yes it does look like there are a few outliers in the pf score hovering around the 0,0 marks

2. For each pair of variables, how would you describe the relationship numerically?
just based off a simple p value calculation where 0.05 shows linearity - we would say that they are in fact not correlated.
...
I do believe that adding the pf_rank variable was unnessecary as it adds no value to whether or not expression is related to pf_score
...
scatterplot3d(
  x = hfi_2016$pf_score,
  y = hfi_2016$pf_expression_control,
  z = hfi_2016$pf_rank,
  color = "blue",   # Set color of points
  main = "3D Scatter Plot",
  xlab = "pf_score",
  ylab = "pf_expression_control",
  zlab = "pf_rank"
)
...
6. Compare your 3-D scatterplot and the `GGally::ggpairs` output.
  ...
  I can only locate the ggpairs plot in github. However I like both visualizations. However, the 3D plot gives you a better visualization - whereas the 2d scatter plot is easier to see linearity.
...
Equation: y = \text{Intercept} + \text{estimate(east_atlanticYes)} \times \text{east_atlanticYes} + \text{estimate(pf_expression_control)} \times \text{pf_expression_control} + \text{estimate(east_atlanticYes:pf_expression_control)} \times \text{east_atlanticYes} \times \text{pf_expression_control}

For the level "east_atlanticNo":
Equation: y = \text{Intercept} + \text{estimate(pf_expression_control)} \times \text{pf_expression_control}
...
If the estimated coefficient for "east_atlanticYes" is positive, it suggests that when the "east_atlantic" level is "Yes," the response variable tends to have higher values. Conversely, if the estimated coefficient for "east_atlanticYes" is negative, it suggests that when the "east_atlantic" level is "Yes," the response variable tends to have lower values.
...
It looks like the PStatistics show good significance to the predicted variable. We could say that quantiative and the qualitative variables can help us predict the pf_score for an overall country lableled east atlantic.
