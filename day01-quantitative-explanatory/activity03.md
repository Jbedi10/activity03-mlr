...
``` r
# Use the table function to get a count of variables by data type
...
hfi_2016 %>% 
  ...
  ggpairs(corner = list(continuous = "cor", p = "p"), 
          upper = list(continuous = "cor", p = "p")) +
  theme_minimal()
```

    ## Warning in warn_if_args_exist(list(...)): Extra arguments: 'corner' are being
...
1.  For each pair of variables, how would you describe the relationship
    graphically? Do any of the relationships look linear? pf score and
    pf expression seem to behave linear Are there any interesting/odd
    features (outliers, non-linear patterns, etc.)? yes it does look
    like there are a few outliers in the pf score hovering around the
    0,0 marks

2.  For each pair of variables, how would you describe the relationship
    numerically? just based off a simple p value calculation where 0.05
    shows linearity - we would say that they are in fact not correlated.
...
3.  Are your two explanatory variables collinear (correlated)?
    ...
    models). I do believe that adding the pf_rank variable was
    unnessecary as it adds no value to whether or not expression is
    related to pf_score
...
4.  Using your output, write the complete estimated equation for this
    model. ^ y = 9.3111 + 0.024012 + -0.03001 Remember in Activity 2
